...
class RandomScaleImageMultiViewImage(object):
    ...
    def __init__(self, scales=[0.5, 1.5], scale_type='list'):
        assert scale_type in ['interval', 'list'], "scale_type must be in [interval, list]!"
        if scale_type == 'interval':
            assert len(scales) == 2, "scale must be 2 if scale_type == interval"
    ...
    def __call__(self, results):
        ...
        results['lidar2img']['extrinsic'] = [scale_factor @ l2i for l2i in results['lidar2img']['extrinsic']]
        ...
        #     img_filename = results['img_info'][ii]['filename'].split('/')[-1]
        #     cv2.imwrite('figs/%s' % img_filename, new_img)
        # exit()
        return results
...
class GlobalRotScaleTrans(object):
    ...
    def update_transform(self, input_dict):
        for cam_id, cam_info in enumerate(input_dict['lidar2img']['lidar2img_aug']):
            ...
            cam_info['tran'] = new_transform[:3, -1]
...
class InternalGlobalRotScaleTrans(GlobalRotScaleTrans):
    def update_transform(self, input_dict):
        # ipdb.set_trace()
        for cam_id, cam_info in enumerate(input_dict['lidar2img']['lidar2img_aug']):
            transform = cam_info['lidar2cam_rt']
...
class TestTimeAugImageMultiViewImage(RandomAugImageMultiViewImage):
    ...
    def __call__(self, results, fix=''):
        ...
        for flip in [False, True]:
            self.data_config['test_flip'] = flip
            for resize in [0.00]:
                self.data_config['test_resize'] = resize

                _results = super().__call__(copy.deepcopy(results), fix=fix)
                results_list.append(_results)

        results = results_list[0]
        for _results in results_list[1:]:
            results['img'].extend(_results['img'])
            results['lidar2img']['extrinsic'].extend(_results['lidar2img']['extrinsic'])
            results['img_shape'].extend(_results['img_shape'])
...
class InternalRandomAugImageMultiViewImage(RandomAugImageMultiViewImage):
    def rts2proj(self, cam_info, post_rot=None, post_tran=None):
        ...
        lidar2cam_rt = cam_info['lidar2cam_rt']
        intrinsic = cam_info['intrin']
...
class InternalRandomAugImageMultiViewImageDebug(InternalRandomAugImageMultiViewImage):
    def __init__(self, repeat=10, *args, **kwargs):
        self.repeat = repeat
        super(InternalRandomAugImageMultiViewImageDebug, self).__init__(*args, **kwargs)
