...
class FastBEV2DExportWrapper(nn.Module):
    ...
    def forward(self, img):
        return self.model(
            img, None, return_loss=False, export_2d=True)
...
class FastBEV3DExportWrapper(nn.Module):
    ...
    def forward(self, x0, x1, x2, x3):
        return self.model(
            (x0, x1, x2, x3), None, return_loss=False, export_3d=True)
...
def main():
    ...
    export_onnx(model, export_2d=True, export_3d=True)

def export_onnx(model, export_2d=False, export_3d=False):
    ...
    if export_2d:
        export_model = FastBEV2DExportWrapper(model).eval()
        ...
        onnx.save_model(simplified_model_2d, f'./simplified_export_2d_model.onnx')

    # 导出3D模型
    if export_3d:
        export_model = FastBEV3DExportWrapper(model).eval()
        inputs = [
            torch.randn((1, 256, 200, 200)).cuda()
            for _ in range(4)
        ]
        onnx_path_3d = f'./export_3d_model.onnx'
        # 导出ONNX模型
        ...
        onnx.save_model(simplified_model_3d, f'./simplified_export_3d_model.onnx')
